...
static int comm_mode;      // Whether to Implement blocking/non-blocking send
static int halo_error = 0; // Checks whether w > m || w > n
static int o = 0;
...
int checkHaloSize(int w) {
  ...
  if (halo_error == 1) {
    ...
    return -1; // Could have used exit(0); here but didn't to gracefully exit
               // from main
  }
  return 0;
}

// Updating boundary non-overlapping case
static void updateBoundary(double *u, int ldu, int w) {
  ...
  if (P == 1) {
    if (verbosity > 1) {
      printAdvectField(rank, "Before UB transfer", M_loc + 2 * w, N_loc + 2 * w,
                       u, ldu);
    }
    ...
    if (verbosity > 1) {
      printAdvectField(rank, "After UB transfer", M_loc + 2 * w, N_loc + 2 * w,
                       u, ldu);
    }
  } else {
    int botProc = (rank + Q) % nprocs,
        topProc = (rank - Q + nprocs) %
                  nprocs; // Think of think as clockwise and anticlockwise
    ...
    */
    ...
    MPI_Type_vector(w, N_loc, N_loc + 2 * w, MPI_DOUBLE,
                    &s_rowtype); // Number of rows being transmitted are w with
                                 // N_loc elements
    MPI_Type_commit(&s_rowtype);
    MPI_Isend(&V(u, M_loc, w), 1, s_rowtype, botProc, HALO_TAG, comm,
              &request[nReq++]);
    MPI_Irecv(&V(u, 0, w), 1, s_rowtype, topProc, HALO_TAG, comm,
              &request[nReq++]);
    MPI_Isend(&V(u, w, w), 1, s_rowtype, topProc, HALO_TAG, comm,
              &request[nReq++]);
    MPI_Irecv(&V(u, w + M_loc, w), 1, s_rowtype, botProc, HALO_TAG, comm,
              &request[nReq++]);
  ...
  if (Q == 1) {
    ...
    int rightProc =
        (Q0 < Q - 1) ? (rank + 1) % nprocs : (rank - Q + 1 + nprocs) % nprocs;
    int leftProc = (Q0 > 0) ? (rank - 1 + nprocs) % nprocs : (rank + Q - 1);
    // printf("LR %d %d %d\n", rank, leftProc, rightProc);
    MPI_Datatype s_coltype;
    int nReq = 0;
    MPI_Request request[4];
    MPI_Type_vector(M_loc + 2 * w, w, N_loc + 2 * w, MPI_DOUBLE,
                    &s_coltype); // Count of each element i
    MPI_Type_commit(&s_coltype);
    if (verbosity > 1) {
      printAdvectField(rank, "Before LR transfer", M_loc + 2 * w, N_loc + 2 * w,
                       u, ldu);
    }

    MPI_Isend(&V(u, 0, N_loc), 1, s_coltype, rightProc, HALO_TAG, comm,
              &request[nReq++]); // w + N_loc - w
    MPI_Irecv(&V(u, 0, 0), 1, s_coltype, leftProc, HALO_TAG, comm,
              &request[nReq++]);
    MPI_Isend(&V(u, 0, w), 1, s_coltype, leftProc, HALO_TAG, comm,
              &request[nReq++]);
    MPI_Irecv(&V(u, 0, w + N_loc), 1, s_coltype, rightProc, HALO_TAG, comm,
              &request[nReq++]);
    MPI_Waitall(nReq, request, MPI_STATUSES_IGNORE);
    if (verbosity > 1) {
      printAdvectField(rank, "After LR transfer", M_loc + 2 * w, N_loc + 2 * w,
                       u, ldu);
    }
  }
} // updateBoundary()
...
// Updating boundary overlapping case
static void updateBoundaryO(double *u, int ldu, MPI_Request *request) {
  ...
  if (P == 1) {
    ...
    MPI_Isend(&V(u, M_loc, 1), N_loc, MPI_DOUBLE, botProc, HALO_TAG, comm,
              &request[nReq++]);
    MPI_Irecv(&V(u, 0, 1), N_loc, MPI_DOUBLE, topProc, HALO_TAG, comm,
              &request[nReq++]);
    MPI_Isend(&V(u, 1, 1), N_loc, MPI_DOUBLE, topProc, HALO_TAG, comm,
              &request[nReq++]);
    MPI_Irecv(&V(u, M_loc + 1, 1), N_loc, MPI_DOUBLE, botProc, HALO_TAG, comm,
              &request[nReq++]);

    // Sending and waiting only for the corner elements
    MPI_Datatype s_corner_type;
    MPI_Type_vector(2, 1, N_loc + 1, MPI_DOUBLE, &s_corner_type);
    MPI_Type_commit(&s_corner_type);
    MPI_Request corner_elem[4];
    int n_cor = 0; // Wait flag only for the corner elements
    MPI_Isend(&V(u, M_loc, 1), 1, s_corner_type, botProc, HALO_TAG, comm,
              &corner_elem[n_cor++]);
    MPI_Irecv(&V(u, 0, 1), 1, s_corner_type, topProc, HALO_TAG, comm,
              &corner_elem[n_cor++]);
    MPI_Isend(&V(u, 1, 1), 1, s_corner_type, topProc, HALO_TAG, comm,
              &corner_elem[n_cor++]);
    MPI_Irecv(&V(u, M_loc + 1, 1), 1, s_corner_type, botProc, HALO_TAG, comm,
              &corner_elem[n_cor++]);
  ...
  if (Q == 1) {
    ...
    int leftProc = (Q0 > 0) ? (rank - 1 + nprocs) % nprocs : (rank + Q - 1);
    MPI_Datatype s_coltype;
    MPI_Type_vector(M_loc + 2, 1, N_loc + 2, MPI_DOUBLE,
                    &s_coltype); // Count of each element i
    MPI_Type_commit(&s_coltype);
    ...
    MPI_Irecv(&V(u, 0, 0), 1, s_coltype, leftProc, HALO_TAG, comm,
              &request[nReq++]);
    MPI_Isend(&V(u, 0, 1), 1, s_coltype, leftProc, HALO_TAG, comm,
              &request[nReq++]);
    MPI_Irecv(&V(u, 0, N_loc + 1), 1, s_coltype, rightProc, HALO_TAG, comm,
              &request[nReq++]);
  }
...
void parAdvectOverlap(int reps, double *u, int ldu) {
  ...
  for (r = 0; r < reps; r++) {
    ...
    // Update inner block
    // Start from (2,2) and end earlier in (M_loc - 1, N_loc -1)
    ...
    // Wait for top/bottom left right exchange except corners
    MPI_Waitall(n_requests, request, MPI_STATUSES_IGNORE);
    // Update white edges manually
...
void parAdvectWide(int reps, int w, double *u, int ldu) {
  ...
  // Number of reps = r * w + r % w
  for (r = 0; r < reps / w; r++) {
    updateBoundary(u, ldu, w);
    // The sequential part :/
    for (w_i = 1; w_i <= w; w_i++) {
      /* perform w updates to the local field of sizes
       * (m+2*w-2)x(n+2*w-2),(m+2*w-4)x(n+2*w-4), ...,(m)x(n) */

      int UR_size = M_loc + (2 * w) - (2 * w_i);
      int UC_size = N_loc + (2 * w) - (2 * w_i);
      updateAdvectField(UR_size, UC_size, &V(u, w_i, w_i), ldu, &V(v, w_i, w_i),
                        ldv);
      copyField(UR_size, UC_size, &V(v, w_i, w_i), ldv, &V(u, w_i, w_i),
                ldu); // TODO correct copying
    }
  }
...
// extra optimization variant (The same as parAdvectOverlap and working with Q >
// 1)
void parAdvectExtra(int reps, double *u, int ldu) {
  ...
  for (r = 0; r < reps; r++) {
    updateBoundaryO(u, ldu, request);
    updateAdvectField(
        M_loc - 1, N_loc - 1, &V(u, 2, 2), ldu, &V(v, 2, 2),
        ldv); // Start from (2,2) and end earlier in (M_loc - 1, N_loc -1)
    if (verbosity > 1) {
      char s[64];
      sprintf(s, "%d reps: u", r + 1);
      printAdvectField(rank, s, M_loc + 2, N_loc + 2, u, ldu);
