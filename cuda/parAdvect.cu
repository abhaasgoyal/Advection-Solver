...
__global__
void updateAdvectFieldOP(int M, int N, double *u, int ldu, double *v, int ldv,
			double Ux, double Uy, int Byy) {
  double cim1, ci0, cip1, cjm1, cj0, cjp1;
  N2Coeff(Ux, &cim1, &ci0, &cip1);
  N2Coeff(Uy, &cjm1, &cj0, &cjp1);
   ...
   * 2. Store (cjm1*V(u,i  ,j-1) + cj0*V(u,i,  j) + cjp1*V(u,i,  j+1)) in V(temp_u,i,j)
   * 3. Do cim1 * V(temp_u, i-1,j)  + ci0 * V(temp_u, i, j) + cip1 * V(temp_u, i+1, j)
   * and store in either temp_u (sync nope) or another shared variable?
  ...
  for (int i = 0; i < i_end + 2; i++) {
      for (int j = 1; j < j_end; j++) {
          shData[Byy * i + j] = cjm1*shData[Byy * i + j - 1] + cj0 * shData[Byy * i + j] + cjp1 * shData[Byy * i + j+1];
      }
  }
  ...
  for (int i = 1; i < i_end; i++) {
      for (int j = 0; j < j_end + 2; j++) {
          shData[Byy * i + j] = cim1 * shData[Byy * (i-1) + j] + ci0 * shData[Byy * i + j] + cip1 * shData[Byy * (i+1) + j];
          }
      }
__syncthreads();
...
void cuda2DAdvect(int reps, double *u, int ldu) {
  ...
  for (int r = 0; r < reps; r++) {
    ...
    updateBoundaryEWP <<<dimG,dimB>>> (M, N, temp_u, ldu); // <<<1,1>>> is also cool cuz stridestuff :/?
    updateAdvectFieldKP <<<dimG,dimB>>> (M, N, &V_(temp_u,ldu,1,1), ldu, &V(v,1,1), ldv,
				  Ux, Uy);
...
void cudaOptAdvect(int reps, double *u, int ldu, int w) {
  ...
  for (int r = 0; r < reps; r++) {
    updateBoundaryNSP <<<dimG,dimB>>> (N, M, temp_u, ldu);
    updateBoundaryEWP <<<dimG,dimB>>> (M, N, temp_u, ldu); // <<<1,1>>> is also cool cuz stridestuff :/?
    updateAdvectFieldOP <<<dimG,dimB, (Bx + 2) * (By + 2) * sizeof(float)>>> (M, N, &V_(temp_u,ldu,1,1), ldu, &V(v,1,1), ldv,
				  Ux, Uy, By);
    copyFieldKP <<<dimG,dimB>>> (M, N, &V(v,1,1), ldv, &V_(temp_u,ldu,1,1), ldu);
  } //for(r...)
  HANDLE_ERROR( cudaMemcpy( u, temp_u, ldv*(M+2)*sizeof(double),
